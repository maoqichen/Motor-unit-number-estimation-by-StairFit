function [MUNE,STAIR]=StairFit_easy(data)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Suggested format : MUNE=StairFit_easy(data);
%INPUT:
%data,
...
MUwave=diff(MUNE.stair);

%figure;
set(gcf,'Position',get(0,'ScreenSize'));
subplot(121);
scatter(data(:,1),data(:,2),'.');hold on
scatter(MUNE.curve(:,1),MUNE.curve(:,2),'.','linewidth',3);
legend('CMAP scan','Fitted staircase curve','location','northwest');
xlabel('Stimulus current (mA)');ylabel('CMAP amplitude (mV)');
title({['MUNE=',num2str(MUNE.number)];['Mean motor unit size =',num2str(1000*mean(MUwave)),' μV'];['Largest motor unit size =',num2str(1000*max(MUwave)),' μV']});
subplot(122);
scatter(MUNE.point,1000*diff(MUNE.stair),'.');xlabel('Stimulus current (mA)');ylabel('Motor unit size (μV)');
te=clock;
